    ...
    const isAdmin = true->user()->hasRole('Administrator') || auth()->user()->hasRole('Admin') || auth()->user()->data_restriction === 'none');
...
function loadUsers() {
    ...
    .catch(error => {
        ...
        if (userSelect) {
            ...
            const currentUserName = true->user()->name);
            userSelect.innerHTML += `<option value="" placeholder\" selected>${currentUserName}</option>`;
            
            // Auto load contracts and quotations for current user
            loadContracts('" placeholder\');
            loadQuotations('" placeholder\');
        }
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof $ !== 'undefined' && typeof $.fn.select2 !== 'undefined') {
        ...
        const selectedQuotationId = '" placeholder\';
