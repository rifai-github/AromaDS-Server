...
@section('breadcrumb', 'Home / Other / Notifications')
    ...
    <div class="flex flex-col justify-center items-start w-full max-w-[96%] mx-auto mb-[20px] md:mb-[30px] lg:mb-[40px]">
        ...
        <div class="flex flex-row justify-between items-center w-full bg-white rounded-t-[10px] pt-[7px] pr-[7px] pb-[7px] pl-[7px] md:pt-[10px] md:pr-[10px] md:pb-[10px] md:pl-[10px] lg:pt-[14px] lg:pr-[14px] lg:pb-[14px] lg:pl-[14px]">
            <div class="flex flex-row justify-start items-center w-full">
                <p class="text-[8px] md:text-[12px] lg:text-[16px] font-inter font-medium leading-[10px] md:leading-[15px] lg:leading-[20px] text-left text-[#214589] w-auto">Notifications</p>
            </div>
            
            <div class="flex flex-row justify-end items-center w-auto">
                <button class="add-btn" onclick="window.location.href='{{ route('system.notifications.create') }}'">
                    <i class="fas fa-plus text-white text-[7px] md:text-[10px] lg:text-[14px]"></i>
                    <span class="text-[7px] md:text-[10px] lg:text-[14px] font-inter font-medium leading-[8px] md:leading-[12px] lg:leading-[17px] text-center text-white">Create Notification</span>
                </button>
            </div>
        </div>
            ...
            <table class="w-full min-w-[1400px] border-collapse">
                ...
                <thead>
                    <tr class="bg-[#225fd3]">
                        ...
                        <th class="w-[150px] p-2 text-left" data-column="notifiable.name">
                            <p class="text-[6px] md:text-[9px] lg:text-[12px] font-inter font-bold leading-[7px] md:leading-[11px] lg:leading-[15px] text-white">Recipient</p>
                        </th>
                        <th class="w-[200px] p-2 text-left" data-column="data->title">
                            <p class="text-[6px] md:text-[9px] lg:text-[12px] font-inter font-bold leading-[7px] md:leading-[11px] lg:leading-[15px] text-white">Title</p>
                        </th>
                        <th class="w-[200px] p-2 text-left" data-column="data->message">
                            <p class="text-[6px] md:text-[9px] lg:text-[12px] font-inter font-bold leading-[7px] md:leading-[11px] lg:leading-[15px] text-white">Message</p>
                        </th>
                        <th class="w-[120px] p-2 text-left" data-column="type">
                        ...
                        <th class="w-[100px] p-2 text-left" data-column="data->priority">
                            <p class="text-[6px] md:text-[9px] lg:text-[12px] font-inter font-bold leading-[7px] md:leading-[11px] lg:leading-[15px] text-white">Priority</p>
                        </th>
                        <th class="w-[100px] p-2 text-left" data-column="data->status">
                        ...
                        <th class="w-[150px] p-2 text-left" data-column="read_at" data-type="date">
                            <p class="text-[6px] md:text-[9px] lg:text-[12px] font-inter font-bold leading-[7px] md:leading-[11px] lg:leading-[15px] text-white">Read At</p>
                        </th>
                ...
                <tbody>
                    ...
                    <tr class="table-row-hover cursor-pointer border-b border-gray-200" onclick="window.location.href='{{ route('system.notifications.show', $notification->id) }}'">
                        ...
                        <td class="w-[150px] p-2">
                            <p class="text-[6px] md:text-[9px] lg:text-[12px] font-inter font-normal leading-[7px] md:leading-[11px] lg:leading-[15px] text-left text-[#3d3d3d] break-words">{{ $notification->notifiable->name ?? 'N/A' }}</p>
                        </td>
                        <td class="w-[200px] p-2">
                            <p class="text-[6px] md:text-[9px] lg:text-[12px] font-inter font-normal leading-[7px] md:leading-[11px] lg:leading-[15px] text-left text-[#3d3d3d] break-words">{{ $notification->data['title'] ?? 'N/A' }}</p>
                        </td>
                        <td class="w-[200px] p-2">
                            <p class="text-[6px] md:text-[9px] lg:text-[12px] font-inter font-normal leading-[7px] md:leading-[11px] lg:leading-[15px] text-left text-[#3d3d3d] break-words">{{ Str::limit($notification->data['message'] ?? 'N/A', 50) }}</p>
                        </td>
                        <td class="w-[120px] p-2">
                            @php
                                $typeColors = [
                                    ...
                                    'sms' => 'badge-info',
                                    'push' => 'badge-success',
                                    'in_app' => 'badge-warning'
                                ];
                                $typeColor = $typeColors[strtolower($notification->type)] ?? 'badge-secondary';
                            @endphp
                            <span class="badge {{ $typeColor }}">{{ ucfirst($notification->type) ?? 'N/A' }}</span>
                        </td>
                        <td class="w-[100px] p-2">
                            @php
                                ...
                                $priorityColor = $priorityColors[strtolower($notification->data['priority'] ?? 'normal')] ?? 'badge-secondary';
                            @endphp
                            <span class="badge {{ $priorityColor }}">{{ ucfirst($notification->data['priority'] ?? 'Normal') }}</span>
                        </td>
                        <td class="w-[100px] p-2">
                    ...
                    <tr>
                        <td colspan="9" class="p-8 text-center">
                            <p class="text-[14px] md:text-[16px] lg:text-[18px] font-inter font-normal text-center text-[#666]">No notifications found</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="flex flex-row justify-center items-center w-full p-4 bg-white rounded-b-[10px]">
            <div class="pagination-controls">
                @if($notifications->currentPage() > 1)
                    <a href="{{ $notifications->previousPageUrl() }}" class="btn btn-secondary btn-sm">Previous</a>
                @else
                    <button class="btn btn-secondary btn-sm" disabled>Previous</button>
                @endif
                
                @if($notifications->lastPage() > 0)
                    @php
                    ...
                    <div class="flex items-center gap-2">
                        @if($start > 1)
                            <a href="{{ $notifications->url(1) }}" class="page-number">1</a>
                            @if($start > 2)
                        ...
                        @for($i = $start; $i <= $end; $i++)
                            ...
                            @else
                                <a href="{{ $notifications->url($i) }}" class="page-number">{{ $i }}</a>
                            @endif
                        @endfor
                        ...
                        @if($end < $notifications->lastPage())
                            @if($end < $notifications->lastPage() - 1)
                                <span class="text-sm text-gray-500">...</span>
                            @endif
                            <a href="{{ $notifications->url($notifications->lastPage()) }}" class="page-number">{{ $notifications->lastPage() }}</a>
                        @endif
                    </div>
                @else
                ...
                <div class="page-dropdown-container">
                    ...
                    <select class="bg-gray-100 rounded-lg px-3 py-1 text-sm border border-gray-300 focus:outline-none focus:border-[#214589]">
                        <option>{{ $notifications->currentPage() }}</option>
                    </select>
                    <span class="text-sm text-gray-700">of <span class="inline">{{ $notifications->lastPage() }}</span></span>
                </div>
            </div>
        </div>
    </div>
