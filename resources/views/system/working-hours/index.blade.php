    ...
    <div class="flex flex-col justify-center items-start w-full max-w-[96%] mx-auto mb-[20px] md:mb-[30px] lg:mb-[40px]">
        ...
        <div class="flex flex-row justify-center items-center w-full p-4 bg-white rounded-b-[10px]">
            <div class="pagination-controls">
                @if(isset($workingHours) && $workingHours->currentPage() > 1)
                    <a href="{{ $workingHours->previousPageUrl() }}" class="btn btn-secondary btn-sm">Previous</a>
                @else
                    <button class="btn btn-secondary btn-sm" disabled>Previous</button>
                @endif
                
                @if(isset($workingHours) && $workingHours->lastPage() > 0)
                    @php
                        $start = max(1, $workingHours->currentPage() - 2);
                        $end = min($workingHours->lastPage(), $workingHours->currentPage() + 2);
                    @endphp
                    
                    <div class="flex items-center gap-2">
                        @if($start > 1)
                            <a href="{{ $workingHours->url(1) }}" class="page-number">1</a>
                            @if($start > 2)
                        ...
                        @for($i = $start; $i <= $end; $i++)
                            @if($i == $workingHours->currentPage())
                                <span class="page-number active">{{ $i }}</span>
                            @else
                                <a href="{{ $workingHours->url($i) }}" class="page-number">{{ $i }}</a>
                            @endif
                        @endfor
                        
                        @if($end < $workingHours->lastPage())
                            @if($end < $workingHours->lastPage() - 1)
                                <span class="text-sm text-gray-500">...</span>
                            @endif
                            <a href="{{ $workingHours->url($workingHours->lastPage()) }}" class="page-number">{{ $workingHours->lastPage() }}</a>
                        @endif
                    </div>
                @else
                ...
                @if(isset($workingHours) && $workingHours->currentPage() < $workingHours->lastPage())
                    <a href="{{ $workingHours->nextPageUrl() }}" class="btn btn-secondary btn-sm">Next</a>
                @else
                ...
                <div class="page-dropdown-container">
                    ...
                    <select class="bg-gray-100 rounded-lg px-3 py-1 text-sm border border-gray-300 focus:outline-none focus:border-[#214589]">
                        <option>{{ $workingHours->currentPage() ?? 1 }}</option>
                    </select>
                    <span class="text-sm text-gray-700">of <span class="inline">{{ $workingHours->lastPage() ?? 1 }}</span></span>
                </div>
            </div>
        </div>
    </div>
