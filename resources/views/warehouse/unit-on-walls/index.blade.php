    ...
    <div class="flex flex-col justify-center items-start w-full max-w-[96%] mx-auto mb-[20px] md:mb-[30px] lg:mb-[40px]">
        ...
        <div class="flex flex-row justify-center items-center w-full p-4 bg-white rounded-b-[10px]">
            <div class="pagination-controls">
                @if(isset($units) && $units->currentPage() > 1)
                    <a href="{{ $units->previousPageUrl() }}" class="btn btn-secondary btn-sm">Previous</a>
                @else
                ...
                @if(isset($units) && $units->hasPages())
                    ...
                    <div class="flex items-center gap-2">
                        @if($start > 1)
                            <a href="{{ $units->url(1) }}" class="page-number">1</a>
                            @if($start > 2)
                        ...
                        @for($i = $start; $i <= $end; $i++)
                            ...
                            @else
                                <a href="{{ $units->url($i) }}" class="page-number">{{ $i }}</a>
                            @endif
                        @endfor
                        ...
                        @if($end < $units->lastPage())
                            @if($end < $units->lastPage() - 1)
                                <span class="text-sm text-gray-500">...</span>
                            @endif
                            <a href="{{ $units->url($units->lastPage()) }}" class="page-number">{{ $units->lastPage() }}</a>
                        @endif
                    </div>
                @else
                ...
                @if(isset($units) && $units->hasMorePages())
                    <a href="{{ $units->nextPageUrl() }}" class="btn btn-secondary btn-sm">Next</a>
                @else
                ...
                <div class="page-dropdown-container">
                    ...
                    <select class="bg-gray-100 rounded-lg px-3 py-1 text-sm border border-gray-300 focus:outline-none focus:border-[#214589]">
                        <option>{{ $units->currentPage() ?? 1 }}</option>
                    </select>
                    <span class="text-sm text-gray-700">of <span class="inline">{{ $units->lastPage() ?? 1 }}</span></span>
                </div>
            </div>
        </div>
    </div>
