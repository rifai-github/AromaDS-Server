    ...
    <div class="flex flex-col justify-center items-start w-full max-w-[96%] mx-auto mb-[20px] md:mb-[30px] lg:mb-[40px]">
        ...
        <div class="flex flex-row justify-center items-center w-full p-4 bg-white rounded-b-[10px]">
            <div class="pagination-controls">
                @if($paymentMethods->currentPage() > 1)
                    <a href="{{ $paymentMethods->previousPageUrl() }}" class="btn btn-secondary btn-sm">Previous</a>
                @else
                    <button class="btn btn-secondary btn-sm" disabled>Previous</button>
                @endif
                
                @if($paymentMethods->lastPage() > 0)
                    @php
                        $start = max(1, $paymentMethods->currentPage() - 2);
                        $end = min($paymentMethods->lastPage(), $paymentMethods->currentPage() + 2);
                    @endphp
                    
                    <div class="flex items-center gap-2">
                        @if($start > 1)
                            <a href="{{ $paymentMethods->url(1) }}" class="page-number">1</a>
                            @if($start > 2)
                        ...
                        @for($i = $start; $i <= $end; $i++)
                            ...
                            @else
                                <a href="{{ $paymentMethods->url($i) }}" class="page-number">{{ $i }}</a>
                            @endif
                        @endfor
                        
                        @if($end < $paymentMethods->lastPage())
                            @if($end < $paymentMethods->lastPage() - 1)
                                <span class="text-sm text-gray-500">...</span>
                            @endif
                            <a href="{{ $paymentMethods->url($paymentMethods->lastPage()) }}" class="page-number">{{ $paymentMethods->lastPage() }}</a>
                        @endif
                    </div>
                @else
                ...
                @if($paymentMethods->currentPage() < $paymentMethods->lastPage())
                    <a href="{{ $paymentMethods->nextPageUrl() }}" class="btn btn-secondary btn-sm">Next</a>
                @else
                ...
                <div class="page-dropdown-container">
                    ...
                    <select class="bg-gray-100 rounded-lg px-3 py-1 text-sm border border-gray-300 focus:outline-none focus:border-[#214589]">
                        <option>{{ $paymentMethods->currentPage() }}</option>
                    </select>
                    <span class="text-sm text-gray-700">of <span class="inline">{{ $paymentMethods->lastPage() }}</span></span>
                </div>
            </div>
        </div>
    </div>
