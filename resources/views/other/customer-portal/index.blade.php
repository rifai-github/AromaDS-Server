...
@section('breadcrumb', 'Home / Other / Customer Portal')
    ...
    <div class="flex flex-col justify-center items-start w-full max-w-[96%] mx-auto mb-[20px] md:mb-[30px] lg:mb-[40px]">
        ...
        <div class="flex flex-row justify-between items-center w-full bg-white rounded-t-[10px] p-4">
            <div class="flex flex-row justify-start items-center w-full">
                <h1 class="text-xl font-semibold text-[#214589]">Customer Portal</h1>
            </div>
            
            <div class="flex flex-row justify-end items-center">
                <button class="btn btn-primary" onclick="openCreateModal()">
                    <i class="fas fa-plus"></i>
                    <span class="hidden md:inline">Add New Portal</span>
        ...
        <div class="table-container">
            <table class="responsive-table">
                ...
                <tbody>
                    ...
                    <tr data-id="{{ $portal->id }}" onclick="openViewModal({{ $portal->id }})">
                        <td class="text-center">
                            <input type="checkbox" class="row-checkbox w-4 h-4 bg-white border border-gray-300 rounded cursor-pointer" value="{{ $portal->id }}" onclick="event.stopPropagation()">
                        </td>
                        <td class="font-medium">{{ $portal->customer->name ?? '-' }}</td>
                        <td>{{ $portal->username ?? '-' }}</td>
                        <td>{{ $portal->portal_url ?? '-' }}</td>
                        <td>
                            <span class="status-badge {{ $portal->is_active ? 'status-active' : 'status-inactive' }}">
                        ...
                        <td>
                            <div class="flex gap-2">
                                ...
                                <button class="btn btn-sm btn-secondary" onclick="event.stopPropagation(); openEditModal({{ $portal->id }})">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button class="btn btn-sm btn-secondary" onclick="event.stopPropagation(); openDeleteModal({{ $portal->id }})">
                                    <i class="fas fa-trash"></i>
                                </button>
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="8" class="p-8 text-center">
                            <div class="text-gray-600">
                                ...
                                <p class="text-lg">No customer portals found</p>
                                <button class="btn btn-primary mt-2" onclick="openCreateModal()">
                                    <i class="fas fa-plus"></i> Add First Portal
                                </button>
                            </div>
                        </td>
                    </tr>
        ...
        <div class="flex flex-row justify-center items-center w-full p-4 bg-white rounded-b-[10px]">
            <div class="pagination-controls">
                @if(isset($portals) && $portals->currentPage() > 1)
                    <a href="{{ $portals->previousPageUrl() }}" class="btn btn-secondary btn-sm">Previous</a>
                @else
                    <button class="btn btn-secondary btn-sm" disabled>Previous</button>
                @endif
                
                @if(isset($portals) && $portals->hasPages())
                    @php
                        $start = max(1, $portals->currentPage() - 2);
                        $end = min($portals->lastPage(), $portals->currentPage() + 2);
                    @endphp
                    
                    <div class="flex items-center gap-2">
                        @if($start > 1)
                            <a href="{{ $portals->url(1) }}" class="page-number">1</a>
                            @if($start > 2)
                        ...
                        @for($i = $start; $i <= $end; $i++)
                            @if($i == $portals->currentPage())
                                <span class="page-number active">{{ $i }}</span>
                            @else
                                <a href="{{ $portals->url($i) }}" class="page-number">{{ $i }}</a>
                            @endif
                        @endfor
                        
                        @if($end < $portals->lastPage())
                            @if($end < $portals->lastPage() - 1)
                                <span class="text-sm text-gray-500">...</span>
                            @endif
                            <a href="{{ $portals->url($portals->lastPage()) }}" class="page-number">{{ $portals->lastPage() }}</a>
                        @endif
                    </div>
                @else
                ...
                @if(isset($portals) && $portals->hasMorePages())
                    <a href="{{ $portals->nextPageUrl() }}" class="btn btn-secondary btn-sm">Next</a>
                @else
                ...
                <div class="page-dropdown-container">
                    ...
                    <select class="bg-gray-100 rounded-lg px-3 py-1 text-sm border border-gray-300 focus:outline-none focus:border-[#214589]">
                        <option>{{ $portals->currentPage() ?? 1 }}</option>
                    </select>
                    <span class="text-sm text-gray-700">of <span class="inline">{{ $portals->lastPage() ?? 1 }}</span></span>
                </div>
            </div>
        </div>
    </div>
...
<div id="modalOverlay" class="modal-overlay" onclick="closeModal()">
    <div class="modal-container" onclick="event.stopPropagation()">
        <div class="modal-header">
            <h2 id="modalTitle" class="modal-title">View Customer Portal</h2>
...
<div id="deleteModalOverlay" class="delete-modal-overlay" onclick="closeDeleteModal()">
    <div class="delete-modal-container" onclick="event.stopPropagation()">
        ...
        <h3 class="delete-modal-title">Hide Customer Portal</h3>
        <p class="delete-modal-description" id="deleteMessage">Apakah kamu yakin ingin menyembunyikan portal ini? Tindakan ini masih bisa dibatalkan nanti.</p>
...
<div id="errorModalOverlay" class="error-modal-overlay" onclick="closeErrorModal()">
    <div class="error-modal-container" onclick="event.stopPropagation()">
        ...
        <p class="delete-modal-description" id="errorMessage">Portal belum berhasil disembunyikan. Silakan coba lagi.</p>
        <div class="delete-modal-buttons">
...
<div id="successModalOverlay" class="success-modal-overlay" onclick="closeSuccessModal()">
    <div class="success-modal-container" onclick="event.stopPropagation()">
        ...
        <p class="delete-modal-description" id="successMessage">Portal berhasil disembunyikan.</p>
    </div>
</div>
...
function openCreateModal() {
    openModal('Create New Customer Portal');
    
    document.getElementById('modalBody').innerHTML = `
        <form id="form" onsubmit="submitForm(event)">
            <div class="modal-section">
                <div class="modal-section-title">Portal Information</div>
    ...
    document.getElementById('modalFooter').innerHTML = `
        ...
        <button type="submit" form="form" class="btn btn-primary">Create Portal</button>
    `;
}

function openViewModal(id) {
    openModal('View Customer Portal');
    ...
    fetch(`/other/customer-portal/${id}`, {
        headers: {
            ...
            document.getElementById('modalBody').innerHTML = `
                <div class="modal-section">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="detail-item">
                            <label class="form-label">Username</label>
                            <p class="detail-value">${data.data.username || '-'}</p>
                        </div>
                        <div class="detail-item">
                            <label class="form-label">Status</label>
                            <p class="detail-value">
                                <span class="status-badge ${data.data.is_active ? 'status-active' : 'status-inactive'}">${data.data.is_active ? 'Active' : 'Inactive'}</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-section">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div class="detail-item">
                            ...
                            <p class="detail-value">${data.data.portal_url || '-'}</p>
                        </div>
                    </div>
                </div>
        ...
        // Add modal footer for view modal
            ...
            <button type="button" class="btn btn-primary" onclick="openEditModal(${id})">Edit Portal</button>
        `;
        })
        .catch(error => {
...
function openEditModal(id) {
    openModal('Edit Customer Portal');
    document.getElementById('modalBody').innerHTML = '<div style="text-align: center; padding: 40px;"><div style="display: inline-block; width: 40px; height: 40px; border: 4px solid #f3f3f3; border-top: 4px solid #3498db; border-radius: 50%; animation: spin 1s linear infinite;"></div><p style="margin-top: 16px; color: #666;">Loading...</p></div>';
    
    fetch(`/other/customer-portal/${id}/edit`, {
        headers: {
            'Accept': 'application/json',
            'X-Requested-With': 'XMLHttpRequest'
        }
        ...
        .then(data => {
                    ...
                    <div class="modal-section">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="form-group">
                                ...
                                <select name="customer_id" class="form-input" required>
                                    <option value="">Select Customer</option>
                                    <option value="${data.data.customer_id}" selected>${data.data.customer?.name || 'Current Customer'}</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label class="form-label">Username *</label>
                                <input type="text" name="username" class="form-input" value="${data.data.username || ''}" placeholder="Enter username" required>
                            </div>
                        </div>
                    </div>
            ...
            document.getElementById('modalFooter').innerHTML = `
                <button type="button" class="btn btn-secondary" onclick="closeModal()">Cancel</button>
                <button type="submit" form="form" class="btn btn-primary">Update Portal</button>
            `;
        })
        .catch(error => {
            console.error('Error:', error);
...
function submitForm(event, id = null) {
    ...
    const url = id ? `/other/customer-portal/${id}` : '/other/customer-portal';
...
function openDeleteModal(id = null) {
    ...
    const message = count === 1 
        ? 'Apakah kamu yakin ingin menyembunyikan portal ini? Tindakan ini masih bisa dibatalkan nanti.'
        : `Apakah kamu yakin ingin menyembunyikan ${count} portal? Tindakan ini masih bisa dibatalkan nanti.`;
...
function confirmDelete() {
    ...
    fetch('/other/customer-portal/bulk-delete', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        },
        body: JSON.stringify({ portal_ids: selectedIdsForRetry })
    })
    .then(response => response.json())
    .then(result => {
        if (result.success) {
    ...
    .catch(error => {
        console.error('Error:', error);
        showErrorModal('Terjadi kendala jaringan');
    });
}
...
function deleteSelected() {
    ...
    if (checkboxes.length === 0) {
        alert('Silakan pilih minimal satu portal untuk disembunyikan');
        return;
    }
...
function showSuccessModal(count) {
    const message = count === 1 
        ? 'Portal berhasil disembunyikan.'
        : `${count} portal berhasil disembunyikan.`;
...
function showErrorModal(message) {
    document.getElementById('errorMessage').textContent = message || 'We couldn\'t hide the portal. Please try again.';
