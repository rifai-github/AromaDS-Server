    ...
    <div class="flex flex-col justify-center items-start w-full max-w-[96%] mx-auto mb-[20px] md:mb-[30px] lg:mb-[40px]">
        ...
        <div class="flex flex-row justify-between items-center w-full bg-white rounded-t-[10px] p-4">
            <div class="flex flex-row justify-start items-center w-full">
                <h1 class="text-xl font-semibold text-[#214589]">Hak Akses</h1>
            </div>
            
            <div class="flex flex-row justify-end items-center">
                <button class="btn btn-primary" onclick="openCreateModal()">
                    <i class="fas fa-plus"></i>
                    <span class="hidden md:inline">Add New Hak Akses</span>
            ...
            <table class="responsive-table">
                ...
                <tbody>
                    ...
                    <tr data-id="{{ $access->id }}" onclick="openViewModal({{ $access->id }})">
                        ...
                        <td class="font-medium">{{ $access->name ?? '-' }}</td>
                        <td>{{ $access->description ?? '-' }}</td>
                        <td>
                            <span class="status-badge {{ $access->regional_table_view ? 'status-active' : 'status-inactive' }}">
                        ...
                        <td>
                            <div class="flex gap-2">
                                <button class="btn btn-sm btn-secondary" onclick="event.stopPropagation(); openViewModal({{ $access->id }})">
                                    <i class="fas fa-eye"></i>
                                </button>
                                <button class="btn btn-sm btn-secondary" onclick="event.stopPropagation(); openEditModal({{ $access->id }})">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button class="btn btn-sm btn-secondary" onclick="event.stopPropagation(); openDeleteModal({{ $access->id }})">
                                    <i class="fas fa-trash"></i>
                                </button>
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="8" class="p-8 text-center">
                            <div class="text-gray-600">
                                ...
                                <p class="text-lg">No hak akses found</p>
                                <button class="btn btn-primary mt-2" onclick="openCreateModal()">
                                    <i class="fas fa-plus"></i> Add First Hak Akses
                                </button>
                            </div>
                        </td>
                    </tr>
...
<div id="modalOverlay" class="modal-overlay" onclick="closeModal()">
    <div class="modal-container" onclick="event.stopPropagation()">
        <div class="modal-header">
            <h2 id="modalTitle" class="modal-title">View Hak Akses</h2>
...
<div id="deleteModalOverlay" class="delete-modal-overlay" onclick="closeDeleteModal()">
    <div class="delete-modal-container" onclick="event.stopPropagation()">
        ...
        <h3 class="delete-modal-title">Hide Hak Akses</h3>
        <p class="delete-modal-description" id="deleteMessage">Apakah kamu yakin ingin menyembunyikan akses ini? Tindakan ini masih bisa dibatalkan nanti.</p>
...
<div id="errorModalOverlay" class="error-modal-overlay" onclick="closeErrorModal()">
    <div class="error-modal-container" onclick="event.stopPropagation()">
        ...
        <p class="delete-modal-description" id="errorMessage">Akses belum berhasil disembunyikan. Silakan coba lagi.</p>
        <div class="delete-modal-buttons">
...
<div id="successModalOverlay" class="success-modal-overlay" onclick="closeSuccessModal()">
    <div class="success-modal-container" onclick="event.stopPropagation()">
        ...
        <p class="delete-modal-description" id="successMessage">Akses berhasil disembunyikan.</p>
    </div>
</div>
...
function openCreateModal() {
    openModal('Create New Hak Akses');
            ...
            <div class="modal-section">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="form-group">
                        ...
                        <select name="regional_table_view" class="form-input">
    ...
    document.getElementById('modalFooter').innerHTML = `
        ...
        <button type="submit" form="form" class="btn btn-primary">Create Hak Akses</button>
    `;
}

function openViewModal(id) {
    openModal('View Hak Akses');
    ...
    fetch(`/other/hak-akses/${id}`, {
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="detail-item">
                            ...
                            <p class="detail-value">
                                <span class="status-badge ${data.data.regional_table_view ? 'status-active' : 'status-inactive'}">${data.data.regional_table_view ? 'Yes' : 'No'}</span>
                            </p>
                        </div>
                        <div class="detail-item">
                            <label class="form-label">Update By</label>
                            <p class="detail-value">${data.data.updater?.name || '-'}</p>
                        </div>
                    </div>
                </div>
        ...
        // Add modal footer for view modal
            ...
            <button type="button" class="btn btn-primary" onclick="openEditModal(${id})">Edit Hak Akses</button>
        `;
        })
        .catch(error => {
...
function openEditModal(id) {
    openModal('Edit Hak Akses');
    document.getElementById('modalBody').innerHTML = '<div style="text-align: center; padding: 40px;"><div style="display: inline-block; width: 40px; height: 40px; border: 4px solid #f3f3f3; border-top: 4px solid #3498db; border-radius: 50%; animation: spin 1s linear infinite;"></div><p style="margin-top: 16px; color: #666;">Loading...</p></div>';
    
    fetch(`/other/hak-akses/${id}/edit`, {
        headers: {
            'Accept': 'application/json',
            'X-Requested-With': 'XMLHttpRequest'
        }
        ...
        .then(data => {
                ...
                <form id="form" onsubmit="submitForm(event, ${id})">
                    ...
                    <div class="modal-section">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="form-group">
                                <label class="form-label">Regional Table View</label>
                                <select name="regional_table_view" class="form-input">
                                    <option value="1" ${data.data.regional_table_view ? 'selected' : ''}>Yes</option>
                                    <option value="0" ${!data.data.regional_table_view ? 'selected' : ''}>No</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="modal-section">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div class="form-group">
                                <label class="form-label">Description</label>
                                <textarea name="description" class="form-input form-textarea" placeholder="Enter description">${data.data.description || ''}</textarea>
                            </div>
                        </div>
                    </div>
                </form>
            ...
            document.getElementById('modalFooter').innerHTML = `
                <button type="button" class="btn btn-secondary" onclick="closeModal()">Cancel</button>
                <button type="submit" form="form" class="btn btn-primary">Update Hak Akses</button>
            `;
        })
        .catch(error => {
            console.error('Error:', error);
...
function submitForm(event, id = null) {
    ...
    const url = id ? `/other/hak-akses/${id}` : '/other/hak-akses';
...
function openDeleteModal(id = null) {
    ...
    const message = count === 1 
        ? 'Apakah kamu yakin ingin menyembunyikan akses ini? Tindakan ini masih bisa dibatalkan nanti.'
        : `Apakah kamu yakin ingin menyembunyikan ${count} akses? Tindakan ini masih bisa dibatalkan nanti.`;
...
function confirmDelete() {
    ...
    fetch('/other/hak-akses/bulk-delete', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        },
        body: JSON.stringify({ access_ids: selectedIdsForRetry })
    })
    .then(response => response.json())
    .then(result => {
        if (result.success) {
...
function deleteSelected() {
    ...
    if (checkboxes.length === 0) {
        alert('Silakan pilih minimal satu akses untuk disembunyikan');
        return;
    }
...
function showSuccessModal(count) {
    const message = count === 1 
        ? 'Akses berhasil disembunyikan.'
        : `${count} akses berhasil disembunyikan.`;
