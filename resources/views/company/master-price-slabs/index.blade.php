    ...
    <div class="flex flex-col justify-center items-start w-full max-w-[96%] mx-auto mb-[20px] md:mb-[30px] lg:mb-[40px]">
        ...
        <div class="flex flex-row justify-center items-center w-full p-4 bg-white rounded-b-[10px]">
            <div class="pagination-controls">
                @if(isset($priceSlabs) && $priceSlabs->currentPage() > 1)
                    <a href="{{ $priceSlabs->previousPageUrl() }}" class="btn btn-secondary btn-sm">Previous</a>
                @else
                    <button class="btn btn-secondary btn-sm" disabled>Previous</button>
                @endif
                
                @if(isset($priceSlabs) && $priceSlabs->hasPages())
                    @php
                        $start = max(1, $priceSlabs->currentPage() - 2);
                        $end = min($priceSlabs->lastPage(), $priceSlabs->currentPage() + 2);
                    @endphp
                    
                    <div class="flex items-center gap-2">
                        @if($start > 1)
                            <a href="{{ $priceSlabs->url(1) }}" class="page-number">1</a>
                            @if($start > 2)
                        ...
                        @for($i = $start; $i <= $end; $i++)
                            @if($i == $priceSlabs->currentPage())
                                <span class="page-number active">{{ $i }}</span>
                            @else
                                <a href="{{ $priceSlabs->url($i) }}" class="page-number">{{ $i }}</a>
                            @endif
                        @endfor
                        
                        @if($end < $priceSlabs->lastPage())
                            @if($end < $priceSlabs->lastPage() - 1)
                                <span class="text-sm text-gray-500">...</span>
                            @endif
                            <a href="{{ $priceSlabs->url($priceSlabs->lastPage()) }}" class="page-number">{{ $priceSlabs->lastPage() }}</a>
                        @endif
                    </div>
                @else
                ...
                @if(isset($priceSlabs) && $priceSlabs->hasMorePages())
                    <a href="{{ $priceSlabs->nextPageUrl() }}" class="btn btn-secondary btn-sm">Next</a>
                @else
                ...
                <div class="page-dropdown-container">
                    ...
                    <select class="bg-gray-100 rounded-lg px-3 py-1 text-sm border border-gray-300 focus:outline-none focus:border-[#214589]">
                        <option>{{ $priceSlabs->currentPage() ?? 1 }}</option>
                    </select>
                    <span class="text-sm text-gray-700">of <span class="inline">{{ $priceSlabs->lastPage() ?? 1 }}</span></span>
                </div>
            </div>
        </div>
    </div>
