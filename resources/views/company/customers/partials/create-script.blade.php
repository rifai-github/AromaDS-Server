<script>
    ...
    /* =========================================
       Inline Contact Modal Functions
       ========================================= */
       
    function openCreateCustomerContactModal(isEditMode = false) {
        loadSalutations();
        loadPositions();
        
        document.getElementById('createCustomerContactForm').reset();
        
        // Always standard create mode behavior for this context
        ...
        document.getElementById('createCustomerContactModalOverlay').classList.add('show');
    }

    function closeCreateCustomerContactModal() {
        document.getElementById('createCustomerContactModalOverlay').classList.remove('show');
    }
        ...
        .then(data => {
            ...
            salutations.forEach(item => {
                if (item.is_active !== false) {
                    const option = document.createElement('option');
                    option.value = item.option_name || item.name || item.title;
                    option.textContent = item.option_name || item.name || item.title;
                    select.appendChild(option);
                }
            });
        });
    ...
    function submitCreateCustomerContactForm() {
        const form = document.getElementById('createCustomerContactForm');
        ...
        .then(data => {
            if (data.success || data.status === 'success') {
                if (typeof showSuccess === 'function') showSuccess(data.message || 'Contact created successfully!');
                else alert(data.message || 'Contact created successfully!');
                
                closeCreateCustomerContactModal();
                
                // Add new option to CREATE Multi PIC
                const createSelect = document.getElementById('create_contact_ids');
                ...
                if (typeof showError === 'function') showError(data.message || 'Failed to create contact');
                else alert(data.message || 'Failed to create contact');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            if (typeof showError === 'function') showError('An error occurred');
