...
/**
 * Change Rental always targets a room that is already installed, so its room is
 * always "used" by the earlier Install Job Advice. If the endpoint filters used
 * rooms out for this type, no room is ever selectable and the JA cannot be made.
 */
class JobAdviceChangeRentalRoomAvailabilityTest extends TestCase
{
    protected function setUp(): void
    {
        ...
        // The room is already consumed by an approved Install JA - exactly the
        // state of any contract that is already running.
        DB::table('job_advices')->insert([
            'id' => 80,
            'job_advice_number' => 'JA/26-05/0001',
            'contract_id' => 20,
            'customer_id' => 10,
            'type' => 'install',
            'status' => 'approved',
    ...
    public function test_change_rental_still_offers_a_room_already_used_by_another_job_advice(): void
    {
        $response = $this->getJson('/api/contracts/20/for-job-advice?type=change_rental');
        ...
        $response->assertOk()
            ...
            ->assertJsonPath('contract_rooms.0.used_by_ja.job_advice_number', 'JA/26-05/0001');
    }

    public function test_change_rental_with_a_space_in_the_type_behaves_the_same(): void
    {
        $response = $this->getJson('/api/contracts/20/for-job-advice?type=change%20rental');
    ...
    public function test_install_still_hides_a_room_already_used_by_another_job_advice(): void
    {
