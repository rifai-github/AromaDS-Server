...
| | Subsequent service — ad-hoc (`service`) | `Service` | CSR |
| | Subsequent service — periodic (`service_routine`) | `Service Routine` | CSR |
| | Remove | `Remove` | RV |
...
- ⚠️ **Those two types carry different labels — do not collapse them into one "Service" row again.** `getDisplayTypeAttribute()` maps `service` → `Service` and `service_routine` → `Service Routine` (plus `service_first` → `Service Pertama (CSR)`). The `Service Routine` string has been in the map since the initial commit and is asserted by `JobScheduleDisplayTypeTest::test_mixed_unit_and_refill_service_still_displays_as_service`. This table used to show a single `Service` row for both, which did not match the code (found 11 Sep 2026 via QA job `SBY-CSR/26-09/0028`, whose `display_type` is `Service Routine`). Whether the client actually wants `Service Routine` as the visible wording was never confirmed — that would be a code + test change needing explicit approval, not a doc edit.
...
- A step that depends on a disabled one still works: `--step=contract_rentals` resolves fine, it just no longer drags `master_rentals` in.
- ⚠️ **Rental mapping needs the `rental_code` fallback — do not route it back through the map.** The `MsProduct → master_rentals` entries in `source_import_maps` are written **only** by the disabled `master_rentals` step, so on any fresh import that map is empty and *every* row of `quotation_rentals`, `quotation_details`, and `contract_rentals` failed with "Master rental missing" — 100%, not a partial skip. That is why imported SQ and contracts had no rentals at all, and why `job_advice_rooms` (which reads those two tables) came out empty too. All three now resolve through `CatalystMasterDataImporter::resolveMasterRentalId()`, which tries the map first and falls back to `master_rentals.rental_code`. The fallback is exact, not approximate: the old `master_rentals` step used `makeKey(ProductCode)` as the map key *and* as `rental_code`. Measured against the live Catalyst source on 14 Sep 2026 — **99.9% of source rows** match (SQ 46 609/46 667, contracts 38 554/38 597; 18 unmatched codes total, several of them junk like `TEST`/`TEST2`), and **zero** codes differ by letter case alone. Locked by `tests/Feature/CatalystMasterRentalFallbackTest.php`. Re-measure any time with `php artisan catalyst:audit-rental-coverage` (read-only; needs the `sqlsrv` driver, or `--skip-source` without it).
- ⚠️ **`quotations` runs before `contracts`, so `existing_contract_id` cannot be resolved inline.** `resolveQuotationExistingContractId()` reads the `MKTContractHd → contracts` map, which is still empty at that point in a from-scratch import, so every renewal SQ got `existing_contract_id = NULL` — the "Nomor Contract" row on the SQ page is guarded on that column and disappeared entirely. The `quotation_existing_contracts` step (placed right after `contracts`) redoes the resolution once the map exists. Do not "simplify" it back into the `quotations` step. Locked by `tests/Feature/CatalystQuotationExistingContractStepTest.php`.
