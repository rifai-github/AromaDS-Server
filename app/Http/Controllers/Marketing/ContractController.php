...
class ContractController extends Controller
{
    ...
    public function getOnWallUnitsForJobAdvice(Request $request)
    {
        try {
            $marketingId = $request->filled('marketing_id') ? (int) $request->marketing_id : null;
            $search = trim((string) $request->get('q', ''));

            $units = UnitOnWall::with(['customer', 'building', 'room.building', 'rental', 'serialNumber'])
                ->whereIn('status', $this->activeUnitOnWallStatusesForJobAdvice())
                ->when($search !== '', function ($query) use ($search) {
                    $query->where(function ($q) use ($search) {
                        $q->where('serial_number', 'like', "%{$search}%")
                            ...
                            ->orWhereHas('room', fn ($roomQuery) => $roomQuery->where('room_name', 'like', "%{$search}%"));
                    });
                })
                ->latest('id')
                ->limit(100)
            ...
            $data = $units
                ->map(function (UnitOnWall $unit) use ($marketingId) {
                    $match = $this->resolveContractForUnitOnWall($unit, $marketingId);
                    ...
                    return [
                        'id' => $unit->id,
                        'serial_number' => $unit->serial_number ?: $unit->serialNumber?->serial_number,
                        'status' => $unit->status,
                        'customer_id' => $unit->customer_id,
                        'customer_name' => $unit->customer?->name ?: $unit->company_name,
                        'building_id' => $unit->building_id ?: $unit->room?->building_id,
                        'building_name' => $buildingName,
                        'room_id' => $unit->room_id,
                        'room_name' => $roomName,
                        'rental_name' => $unit->rental?->rental_name ?: $unit->rental_name,
                        'contract_id' => $contract->id,
                        'contract_number' => $contract->contract_number,
                        'contract_room_id' => $contractRoom?->id,
                        'marketing_id' => $contract->marketing_id ?: $contract->created_by,
                    ];
                })
                ->filter()
                ->values();
            ...
            Log::error('Failed to load Unit On Wall contract candidates for Job Advice Extra: '.$e->getMessage(), [
                'error' => $e->getMessage(),
    ...
    private function resolveContractForUnitOnWall(UnitOnWall $unit, ?int $marketingId = null): ?array
    {
        if (! $unit->customer_id) {
            return null;
        }
        ...
        $contracts = Contract::with(['customer', 'contractRooms.room'])
            ->where('customer_id', $unit->customer_id)
            ->where(function ($query) {
                $query->where('status', 'active')
                    ->orWhere('contract_status', 'active');
            })
            ->when($marketingId, function ($query) use ($marketingId) {
                $query->where(function ($q) use ($marketingId) {
                    $q->where('marketing_id', $marketingId)
                        ->orWhere('created_by', $marketingId);
                });
            })
            ->whereHas('contractRooms', function ($query) use ($roomId, $buildingId, $normalizedRoomName) {
                $query->where(function ($roomQuery) use ($roomId, $buildingId, $normalizedRoomName) {
                    if ($roomId) {
                        $roomQuery->where('room_id', $roomId);
                    }

                    if ($buildingId && $normalizedRoomName !== '') {
                        $roomQuery->orWhereHas('room', function ($masterRoomQuery) use ($buildingId, $normalizedRoomName) {
                            $masterRoomQuery->where('building_id', $buildingId)
                                ->whereRaw('LOWER(TRIM(room_name)) = ?', [$normalizedRoomName]);
                        });
                    }
                });
