...
class JobAssignMaterialIssueController extends Controller
{
    ...
    private function buildRentalQuantityMultiplierMap(?\App\Models\JobSchedule $jobSchedule): array
    {
        ...
        $register = function ($jaRoom) use (&$map) {
            ...
            // Floor at 1 to stay in sync with autoCreateMaterialIssue(): a legacy contract row
            // with qty 0 still gets material, so its BOM target must not collapse to 0 either
            // (a 0 target silently skips the Submit-to-Issue BOM validation entirely).
    ...
    private function buildIndexRentalQtyMap($issues): array
    {
        ...
        foreach ($issues as $issue) {
            ...
            $register = function ($jaRoom) use (&$map, $jobAssignScheduleId) {
                ...
                // Floor at 1 — see buildRentalQuantityMultiplierMap().
                $qty = max(1, (int) round((float) ($jaRoom->operational_quantity ?? 1)));
                $map[$key] = max($map[$key] ?? 1, $qty);
            };
    ...
    private function saveMaterialIssueItems(MaterialIssue $materialIssue, JobAssignMaterialIssue $jobAssignMaterialIssue, ?array $formData = null)
    {
        ...
        foreach ($assignedRooms as $jaRoom) {
            ...
            // and stays in sync with autoCreateMaterialIssue (which also floors at 1).
