...
class JobScheduleController extends Controller
{
    ...
    public function show(JobSchedule $jobSchedule)
    {
        ...
        if (\Illuminate\Support\Facades\Schema::hasTable('inventory_issuing_item_serials')) {
            $inventoryIssuingRelations[] = 'items.serialLinks.serialNumber';
        }
        ...
        $inventoryIssuings = $materialIssueNumbers->isNotEmpty()
            ? \App\Models\InventoryIssuing::whereIn('reference_no', $materialIssueNumbers->all())
                ->with($inventoryIssuingRelations)
                ->orderBy('id')
                ->get()
            : collect();
        ...
        if (in_array(strtolower($jobSchedule->type), ['remove', 'remove_free', 'remove free'])) {
            ...
            $issuedItems = \App\Models\InventoryIssuingItem::whereHas('inventoryIssuing', function($q) use ($materialIssueNumbers) {
    ...
    protected function autoCreateMaterialIssue($jobAssignSchedule, $specificRoomIds = [])
    {
        ...
        foreach ($itemsToProcess as $item) {
            ...
            //
            // The floor of 1 is deliberate: presence of a rental line in the Job Advice IS the
            // operational decision to service it, so material must be issued even when the
            // contract row says qty 0. ~1000 legacy contract_rentals carry quantity=0 AND
            // qty_free=0 (Catalyst import artifacts that bypassed the app's own validation,
            // which rejects both-zero) — honouring that 0 literally silently generates no
            // material at all for those rooms. Do not "fix" this back to max(0, ...).
